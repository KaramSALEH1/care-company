...
@section('content')
    <section class="bg-white rounded-none shadow-lg p-10 w-full">
        <h1 class="text-3xl font-extrabold text-blue-800 mb-6">Add New Product</h1>
        <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            <div>
                <label class="block mb-1 font-semibold text-blue-700" for="name">Name</label>
                <input class="w-full border border-blue-200 rounded px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition" type="text" id="name" name="name" value="{{ old('name') }}" required>
            ...
            <div>
                ...
                <input class="w-full border border-blue-200 rounded px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition" type="text" id="slug" name="slug" value="{{ old('slug') }}" required>
            ...
            <div>
                ...
                <textarea class="w-full border border-blue-200 rounded px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 transition" id="description" name="description" rows="4" required>{{ old('description') }}</textarea>
            ...
            <button type="submit" class="w-full bg-blue-700 text-black px-6 py-2 rounded-full font-semibold shadow hover:bg-blue-800 transition">Create Product</button>
